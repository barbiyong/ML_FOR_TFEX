...
def get_y_long(data, TP, cutloss, p_train):
    slice = 500
    cutloss = cutloss * -1
    ...
    #     if i == 1:
    #         count = count + 1
    # print(count, len(ret_list), round(count / len(ret_list) * 100, 2))
    #
    # # train plott
    # # os.remove('train_data.html')
    # output_file("train_data.html", title='train_data_'+str(TP))
...
def get_y_long_period(data, TP, cutloss, p_train, period):
    slice = 500
    cutloss = cutloss
    data = [float(i) for i in data]
    ret_list = []
...
def get_x(ohlc):
    ...
    df = pd.DataFrame(
        {'CLOSE': ohlc[2][slice:], 'UBB': UBB[slice:], 'MBB': MBB[slice:], 'LBB': LBB[slice:], 'EMA5': EMA5[slice:], 'EMA10': EMA10[slice:], 'EMA25': EMA25[slice:],
         'EMA50': EMA50[slice:], 'EMA75': EMA75[slice:], 'EMA200': EMA200[slice:], 'RSI7': RSI7[slice:], 'MACD': MACD[slice:], 'HIST': HIST[slice:], 'PDI': PDI[slice:],
         'NDI': NDI[slice:], 'ADX': ADX[slice:]})

    return df


def get_x_v2(ohlc):
    ohlc_df = pd.DataFrame({'Close': ohlc[2], 'Open': ohlc[1], 'High': ohlc[3], 'Low': ohlc[4]})
    slice = 500
    # ema
    EMA5 = get_ema(closes=ohlc[2], period=5)
    EMA10 = get_ema(closes=ohlc[2], period=10)
